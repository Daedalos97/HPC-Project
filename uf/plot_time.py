...
def generate_data(perc_type, prob):
        ...
        plotPref.write("set title 'CPU time vs. Lattice size for sequential and multithreaded solutions using Union Find.'\n")
        ...
        for i in range(2, 13):
            ...
            avg_mt = 0.0
            for j in range(0, 5):
                exe_seq = "./percolation -l "+ str(latsiz)+ " -p "  + str(prob) +" -" + str(perc_type)
                exe_mt =  "./percolation -l "+ str(int(math.pow(2, i)))+ " -p "  + str(prob) +" -" + str(perc_type) +" -m"
